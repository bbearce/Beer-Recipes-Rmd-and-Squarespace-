...
hops <- data.frame(
  ...
  Alpha_Acid_Percent = c(9.5/100,9.5/100), #percent stored as decimal
  Weight_Oz = c(.5,.5),
  Boil_Time = c(60,15)
)
...
GT <- 70
# Target Mash Temp
TMT <- 149 # While 149 is our first temp, the mash out vol will be based on our second temp
TMT2 <- 158# Second mash temp
# Mash Duration
MD <- 60
...
IT = (0.2/MT)*(TMT-GT) + TMT
# Mash Out Volume
MOV = ( (170 - TMT2)*(0.2*GB + MV) )/(212 - 170)  #------ TMT2 IS BEING USED...ADJUST ACCORDINGLY ------#
# Sparge volume post mash out
...
<strong>Sacc Rest Temp1:</strong> `r TMT`  
<strong>Sacc Rest Temp2:</strong> `r TMT2`  
<strong>Mash Duration:</strong> Two temps at 30 min each for a total of: `r MD` min  
...
* Heat `r MV` Gallons up to `r 161.64` degrees Fahrenheit.
* Slowly pour the grains into the water as to avoid clumping.
* Start to raise `r SV` to 170 degrees F. Separate the `r SV` gallons into the mash out volume (`r MOV`) to be brought to a boil, and rinse volume (`r SVPMO`), which should reach 170.
* Check that the mash temp is `r TMT`. Hold here for 25 minutes. Then turn the burners on and raise to `r TMT2`.
* Hopefully the mash temperature raise only takes 5 minutes. Hold at `r TMT2` for 30 minutes. 
* Our total sparge volume is the `r SV` gallons. `r MOV` gallons of that are for mash out. Hopefully that is close to 212 degrees by now, and pour in. 
...
* Once fully drained, discard grains. 
* For the next 30-45 minutes bring to a boil. Once the wort is a 212 degrees F, set 60 minute timer. 
* Add the first hop addition (`r hops$Hop[1]`) at the beginning of the boil. Begin to hydrate the yeast.
...
* After 45 minutes add the second hop addition (`r hops$Hop[2]`). Finsh the boil in 15 minutes for a full 60 minute boil.
* Gather sanitation and prepare buckets.
* Yeast should be foamy. It is ready but we need to cool the wort prior to pitching.
* Cool wort till it reaches `r fermentation$Temp1` degrees F. Transfer to bucket and take OG reading.
* Pitch yeast and put beer in fridge for 28 days at `r fermentation$Temp1` degrees F.
